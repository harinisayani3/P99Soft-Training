...
export const getAllPorts = (req, res) => {
    const { country, status, port_role, limit, page } = req.query;
    let result = [...ports];
    if (country) {
...
export const createPort = (req, res) => {
    ...
    if (missing.length > 0) {
        return fail(res, `Missing required field(s): ${missing.join(",")}`, 404);
    }
    ...
    if (exists) {
        return fail(res, `A port with unlocode "${unlocode}" already exists`, 404)
    };

    const newPort = { ...req.body, unlocode: unlocode.toLowerCase() };
    ports.push(newPort);
    ok(res, newPort, 201);
}
